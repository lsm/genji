...
!(function () {
  ...
  function Model(name, properties, staticProperties) {
    ...
    function _validateField(ModelCtor, fieldName, value) {
      ...
      var field = ModelCtor._maps[fieldName] || fieldName;
      var validator = ModelCtor._fields[field];
    ...
    ModelConstructor.prototype = {
      ...
      getInvalidFields:function () {
        return this._invalidFields.count === 0 ? false : this._invalidFields.fields;
      },
      ...
      toDoc:function (keys) {
        ...
        var _maps = this._ctor._maps;
        keys.forEach(function (key) {
          doc[_aliases[key] || key] = self.attr(_maps[key] || key);
        });
        return doc;
      }
    };

    ModelConstructor.toData = function (doc, keys) {
      var m = new ModelConstructor(doc);
      return m.toData(keys);
    };

    ModelConstructor.toDoc = function (doc, keys) {
      var m = new ModelConstructor(doc);
      return m.toDoc(keys);
    };
