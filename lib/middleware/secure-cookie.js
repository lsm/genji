  ...
  make: function(conf) {
      ...
      if (headers.hasOwnProperty(cookieName)) {
        ...
        var o = headers[cookieName],
          ...
          // encrypt signed cookie into base64 string
            encryptedBase64 = cipher(signed, secureKey, {outputEncoding: 'base64'});
        cookieOption.expires = o[1];
        // set other cookie option if any
        if (o[3]) {
          var option = o[3];
          if (option.domain) cookieOption.domain = option.domain;
          if (option.path) cookieOption.path = option.path;
        }
        // make the cookie
    ...
    return function(req, res, go) {
      ...
      if (cookie) {
        ...
        var data = dcrypted[2];
        try {
          data = JSON.parse(dcrypted[2]);
        } catch(e) {}
        this.userObj = {
          id: dcrypted[0],
          expires: new Date(dcrypted[1]),
          data: data
        };
        // for later usage
