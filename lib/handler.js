...
var BaseHandler = Base(EventEmitter, {
  ...
  redirect: function(url, permanent, noCookie) {
    if (noCookie) this._cookies = [];
    this.setStatus(permanent ? 301 : 302);
    this.setHeader("Location", url).finish();
  },
    ...
    if (request.method === 'POST' || request.method === 'PUT') {
      ...
      request.on("end", function() {
        ...
        if (self.listeners('params').length > 0) {
          var params = querystring.parse(buff);
          self.data.params = params;
          self.emit('params', params, buff);
        } else if (self.listeners('json').length > 0) {
          try {
            var json = JSON.parse(buff);
            self.data.json = json;
            self.emit('json', json, buff);
          } catch(e) {
