...
function App(name, properties, staticProperties) {
  ...
  function AppConstructor(options, properties, staticProperties) {
    ...
    // name of the app
    this.name = _name;
    // access app instance through `this.app`
    this.app = this;
    // url prefix (root)
    ...
    if (!options_.urlRoot) {
      ...
      options_.urlRoot = '^/' + this.name.toLowerCase();
    }
    this.urlRoot = options_.urlRoot;
    if (this.urlRoot[0] !== '^') {
    ...
    Object.keys(AppConstructor._routes).forEach(function (routeName) {
      _routes[routeName] = extend({}, AppConstructor._routes[routeName]);
    });
  ...
  AppConstructor.prototype.onRouteResult = function (event, listener) {
    ...
    if (typeof listener === 'function') {
      ...
      if (['json', 'html', 'text'].indexOf(listener)) {
        throw new Error('Route result shortcuts support only: "json", "html", "text".');
      }
      listener_ = function (handler, err, data) {
        switch (listener) {
          case 'json':
      ...
      throw new Error('Route result listener should be ether function or shortcuts string (e.g. json, html).');
    }
    this.app.on(routeEvent, listener_);
    return this;
  };
  ...
  AppConstructor.prototype.processRoutes = function () {
    ...
    Object.keys(self._routes).forEach(function (routeName) {
      ...
      if (!route.handleFunction) {
        route.handleFunction = function handleFunction(handler) {
          ...
          var _emit = function (event) {
            ...
            var emitted1 = self.emit.apply(self, args);
            // emit the normal result event with oringinal event name
            // shift the modified event name
            args.shift();
            // override `handler` with event name
            args[0] = event;
            var emitted2 = self.emit.apply(self, args);
            return emitted1 || emitted2;
          };
          var businessFnContext = {
            emit:_emit,
            app:self
          };
          if (handler.account) businessFnContext.session = handler.account;
          var fn = self[routeName];
          if (typeof fn === 'function') {
            if (['post', 'put', 'delete'].indexOf(route.method) > -1) {
              // `post` alike http request
              // @todo handle when post to a url like 'http://example.com/?x=1'
              if (handler.data) {
                var input = handler.data.params || handler.data.json;
                args.push(handler.params ? extend(handler.params, input) : input);
                handler.data.raw && args.push(handler.data.raw);
                // your business logic function
                typeof fn === 'function' && fn.apply(businessFnContext, args);
              } else {
              ...
              if (typeof fn === 'function') {
                handler.params && args.push(handler.params);
                typeof fn === 'function' && fn.apply(businessFnContext, args);
              }
            }
          } else {
            console.log('Warning: Your app "%s" dose not contains a function called "%s"', self.name, routeName);
          }
        };
      } else {
        var handleFunction = route.handleFunction;
        route.handleFunction = function (handler) {
          var args = toArray(arguments);
          var _emit = function (event) {
            var args = toArray(arguments);
            // first, shift off event name
          ...
          if (handler.account) businessFnContext.session = handler.account;
          handleFunction.apply(businessFnContext, args);
        };
      }
      if (typeof route.url === 'string' && route.url[0] !== '^') {
        route.url = self.urlRoot + route.url;
      }
      self.routes.push(route);
    });
  };

  AppConstructor.prototype.routePreHook = function (hooks, routeSpecificHook) {
    var hooks_ = hooks;
    // app level route prehook
    typeof hooks === 'function' && (hooks_ = [hooks]);
    var self = this;
    var routes = self._routes;
    if (Array.isArray(hooks_)) {
      // set prehook for all routes
      hooks_.forEach(function (hook) {
        var t = typeof hook;
        if (t === 'function') {
          Object.keys(routes).forEach(function (routeName) {
            var route = routes[routeName];
            route.hooks || (route.hooks = {pre: []});
            route.hooks.pre.push(hook);
          });
        } else if (t === 'string') {
          self.routePreHook(hook, routeSpecificHook);
        } else {
          throw new Error('`hooks` must be functions or names of routes.');
        }
      });
    } else if (typeof hooks === 'string') {
      // prehook for specific route
      var route = routes[hooks];
      if (!route) {
        throw new Error('Route "' + hooks + '" not exists.');
      }
      if (typeof routeSpecificHook !== 'function') {
        throw new Error('Route prehook must be function.');
      }
      route.hooks || (route.hooks = {pre: []});
      route.hooks.pre.push(routeSpecificHook);
    }
  };
  ...
  function processResultEvents(app) {
    ...
    results && Object.keys(results).forEach(function (resultEvent) {
      ...
      if (Array.isArray(resultListener)) {
        resultListener.forEach(function (listener) {
          app.onResult(resultEvent, listener);
        });
      } else {
        app.onResult(resultEvent, resultListener);
      }
    });
  }
  ...
  Object.keys(properties).forEach(function (propKey) {
    ...
    switch (propKey) {
      case 'results':
        extend(AppConstructor._results, property);
        break;
      case 'routeResults':
        extend(AppConstructor._routeResults, property);
        break;
      case 'routes':
        extend(AppConstructor._routes, property);
        break;
      default:
        ...
        if (typeof property === 'function') {
          ...
          property = function () {
            ...
            if (typeof lastArg === 'function') {
              // last argument is a callback function, replace `emit` with callback
              var self = this;
              var emit = function (event) {
                var args = toArray(arguments);
                args.shift();
                lastArg.apply(self, args);
              };
              return fn.apply({app:this, emit:emit}, args);
            } else {
              args.push(lastArg);
        ...
        AppConstructor.prototype[propKey] = property;
        break;
    }
  });
