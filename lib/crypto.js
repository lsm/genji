...
module.exports = {
  ...
  cipher: function(plain, key, options) {
    ...
    var c = crypto.createCipher(options.algorithm || 'aes256', new Buffer(key));
  ...
  decipher: function(cipher, key, options) {
    ...
    var d = crypto.createDecipher(options.algorithm || 'aes256', new Buffer(key));
    var result = '';
    try {
      result = d.update(cipher, options.inputEncoding || 'hex', oe) + d.final(oe);
    } catch (e) {}
    return result;
  },
