 ...
 *   - "defaultHandlerClass" Default handler constructor class
...
function Router(routes, options) {
  this.rules = [];
  options = options || {};
  this.defaultHandlerClass = options.defaultHandlerClass;
  this.urlRoot = options.urlRoot || '^/';
  if (this.urlRoot[0] !== '^') this.urlRoot = '^' + this.urlRoot;
  if (this.urlRoot[this.urlRoot.length - 1] !== '/') this.urlRoot += '/';
...
Router.prototype = {
  ...
  add: function (method, pattern, handleFunction, options) {
    ...
    options = options || {};
    var handlerClass = options.handlerClass || this.defaultHandlerClass;
    ...
    if (Array.isArray(handleFunction) && Array.isArray(handleFunction[0])) {
      ...
      var subRouter = new Router(routes, {defaultHandlerClass: handlerClass});
   ...
   * @param handleFunction {Function} Function to handle the request
   * @param [options] {Object|Array|Function} An optional object for advanced customization:
   *  - Object: a object that contains `hooks` or `handlerClass`
   *  - Array: an array of hook functions
   *  - Function: a handlerClass constructor
   */

  get: function (url, handleFunction, options) {
    this.mount([[url, handleFunction, 'get', options]]);
  },
  ...
  post: function (url, handleFunction, options) {
    this.mount([[url, handleFunction, 'post', options]]);
  },
  ...
  put: function (url, handleFunction, options) {
    this.mount([[url, handleFunction, 'put', options]]);
  },
  ...
  delete: function (url, handleFunction, options) {
    this.mount([[url, handleFunction, 'delete', options]]);
  },
  ...
  head: function (url, handleFunction, options) {
    this.mount([[url, handleFunction, 'head', options]]);
  },
  ...
  notFound: function (url, handleFunction, options) {
    this.mount([[url, handleFunction, 'notFound', options]]);
  },
   ...
   *
   * @param server {HttpServer} HttpServer instance of nodejs
   * @param [notFound] {Function} Optional function to handle miss matched url
   * @public
   */

  listen: function (server, notFound) {
    var self = this;
    ...
    server.on('request', function (request, response) {
      ...
      self.route(request.method, request.url, response, notFound);
    });
  },
