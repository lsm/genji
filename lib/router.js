...
Router.prototype = {
  ...
  add: function (method, pattern, handleFunction, options) {
    ...
    if (Array.isArray(handleFunction) && Array.isArray(handleFunction[0])) {
      ...
      routes.forEach(function (url, idx) {
        ...
        if (hooks) {
          def.options.hooks = def.options.hooks || {};
          if (hooks.pre) {
            def.options.hooks.pre = hooks.pre.concat(def.options.hooks.pre || []);
          }
          if (hooks.post) {
            def.options.hooks.post = (def.options.hooks.post || []).concat(hooks.post);
          }
        }
      ...
      if (hooks) {
        if (hooks.pre) {
          handleFunction = hooks.pre.concat(handleFunction);
        }
        if (hooks.post) {
          handleFunction = handleFunction.concat(hooks.post);
        }
  ...
  normalizeRouteDefinition: function (route, defaults) {
    ...
    options = {hooks: hooks, handlerClass: handlerClass};

    if (hooks && !hooks.pre && !hooks.post) {
      ...
      var nullPlaceholder = hooks.indexOf(null);

      if (nullPlaceholder > 0) {
        var _hooks = {};
        _hooks.pre = hooks.slice(0, nullPlaceholder);
        if (++nullPlaceholder < hooks.length) {
          _hooks.post = hooks.slice(nullPlaceholder);
        }
        hooks = _hooks;
      } else if (nullPlaceholder === 0) {
        hooks.shift();
        hooks = {post: hooks};
      } else {
        hooks = {pre: hooks};
