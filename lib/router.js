...
Router.prototype = {
  ...
  mount: function (routes) {
    ...
    routes.forEach(function (route) {
      ...
      self.add(def.method, def.pattern, def.handleFunction, def.hooks);
    });
  },
   ...
   * @param handleFunction {Function} Function that will be called during dispatch
  ...
  add: function (method, pattern, handleFunction, hooks) {
    ...
    if (Array.isArray(handleFunction) && Array.isArray(handleFunction[0])) {
      ...
      routes.forEach(function (url, idx) {
        ...
        routes[idx] = [def.pattern, def.handleFunction, def.method, def.hooks];
      });
  ...
  get: function (url, handleFunction, hooks) {
    this.mount([[url, handleFunction, 'get', hooks]]);
  },
  ...
  post: function (url, handleFunction, hooks) {
    this.mount([[url, handleFunction, 'post', hooks]]);
  },
  ...
  put: function (url, handleFunction, hooks) {
    this.mount([[url, handleFunction, 'put', hooks]]);
  },
  ...
  delete: function (url, handleFunction, hooks) {
    this.mount([[url, handleFunction, 'delete', hooks]]);
  },
  ...
  head: function (url, handleFunction, hooks) {
    this.mount([[url, handleFunction, 'head', hooks]]);
  },
  ...
  notFound: function (url, handleFunction, hooks) {
    this.mount([[url, handleFunction, 'notFound', hooks]]);
  },
   ...
   * @public
   */
  generateCamelCaseUrl: function (name) {
    var url = '';
    for (var idx in name) {
      var lowCase = name[idx].toLowerCase();
      if (name[idx] === lowCase) {
        url += lowCase;
      } else {
        url += '/' + lowCase;
  ...
  normalizeRouteDefinition: function (route, defaults) {
    ...
    return {pattern: route[0], handleFunction: route[1], method: method, hooks: hooks};
  },
    ...
    rules.forEach(function (rule) {
      ...
      if (hasSub) {
        ...
        current.dispatch = function (matched, context) {
          matched[0] = context;
          handleFunction.apply(null, matched);
        };
        current.match = function (condition) {
