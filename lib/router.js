...
function Router(routes, options) {
  ...
  this.shouldInjectDependency = options.shouldInjectDependency || false;
          ...
          if (_routes.hasOwnProperty(i)) {
            ...
            if (matched !== null) {
              if (Array.isArray(matched)) {
                ...
                matched[0] = context;
                _route.dispatch.apply(context, matched);
                return true;
  ...
  build: function (rules) {
    ...
    rules.forEach(function (rule) {
      ...
      if (hasSub) {
        ...
        current.dispatch = self.shouldInjectDependency ? self.injector.inject(handler) : handler;
        current.match = function (condition) {
