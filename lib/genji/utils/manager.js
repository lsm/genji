...
function startServer(settings) {
    ...
    settings.appPath = path.dirname(process.mainModule.filename);
    settings.genjiPath = genjiPath;
    setLevel(settings.level || 1, settings.env);
    genji.settings = settings;
    if (settings.env === 'development' && exports.optionParse('dev')) {
        watchDir(settings.appPath);
        runScript(process.mainModule.filename, true);
    } else {
