...
var defaultSettings = exports.defaultSettings = {
    ...
    ,middlewares: [
        ...
        {name:'logger'},
        {name: 'redirect'},
        {name: 'router', conf: {handlerClass: 'genji.web.handler.Simple', urls: './app'}}
    ]
}
...
function runServer(settings) {
    ...
    genji.settings = settings || defaultSettings;
    var appDir = path.dirname(process.mainModule.filename);
    var options = settings.servers[0];
    if (settings.env) {
        watchDir(appDir);
