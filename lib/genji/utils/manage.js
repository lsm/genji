...
function run_server(settings) {
    ...
    if (settings.hasOwnProperty('middleware')) {
        options.middleware = settings.middleware;
    }
    if (exports.optionParse('dev')) {
        watchDir(appDir);
        run_script(process.mainModule.filename, true);
    } else {
        ...
        genji.servers = [genji.web.run(urls, handlerClass, options)];
    }
}
