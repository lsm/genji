 ...
 * Module dependencies
 */
var toArray = require('./misc').toArray;
...
function parallel(array, callback, doneCallback) {
    ...
    array.forEach(function(item, idx, arr) {
        if (count < len) {
            callback(item, idx, arr, function(res, finish) {
                count++;
                result[idx] = res;
                if (finish === true || count === len) {
                    doneCallback(result);
                    count = len;
                }
            });
        ...
        args.push(function(err) {
            ...
            if (!err) {
                ...
                if (andCallback.length > 0) {
                    // call the `and` stack
                    chain(andCallback, function(fn, idx, fnChain, next, args) {
                        this.next = next;
                        fn.apply(this, args) && next();
