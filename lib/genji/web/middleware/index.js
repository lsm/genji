...
exports.makeFlaker = function(settings, flakes) {
    ...
    flakes.forEach(function(m) {
        ...
        try {
            ...
            var f = flake.make.call(flaker, settings, m.conf);
            if (f) {
                names[midx++] = m.name;
                stack[m.name] = f;
            }
        } catch(e) {
            // @todo no event named `error`
            flaker.emit('error', {
                exception: e,
                code: 500,
    ...
    function go(idx, ctx, request, response, where, jumping) {
        ...
        try {
            ...
            flaker.emit('error', {
                exception: e,
                code: 500,
                message: 'Your middleware `'+ names[idx] +'` threw exception.',
                request: request,
                response: response
            });
        }
    ...
    return function(request, response) {
        ...
        var ctx = {
            // you can use this object to share data between middlewares in current session
            share: {},
            // http.serverRequest and http.serverResponse object emitted from 'request' event of http.Server
            request: request,
            response: response,
            // the `writeHead`, `write`, `end` chain,
            // corresponding functions of request object will be called in the last step
