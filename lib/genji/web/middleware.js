...
var Flaker = function(request, response, globalEmitter) {
    ...
    // register events
    this.once('#writeHead', function(statusCode, headers) {
        response.writeHead(statusCode || this._statusCode, headers || this._headers);
    });
}
...
exports.makeFlaker = function(conf, flakes) {
    var stack = {};
    var names = [];
    ...
    return function(request, response) {
        ...
        function go(where, jumping) {
            ...
            if (where === undefined) {
                ...
                if (!curr) {
                    throw new Error("End of stack, you should have at least one handler that don't call `go`.");
                }
            } else {
