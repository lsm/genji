...
function Router(urls, defaultHandler, parentType) {
    ...
    var _fromDeclared = function(urls) {
        urls.forEach(function (url) {
            var pattern = url[0], handleFunction = url[1], method = url[2], handlerClass, hooks;
            var offset = typeof method == 'string' ? 0 : -1;
            if (typeof method == 'string') {
                offset = 0;
            } else {
                offset = -1;
                method = parentType ? parentType : 'GET';
            }
...
Router.prototype = {
            ...
            if (typeof handleFunction[0] == 'function') {
                ...
                if (preHooks) {
                    handleFunction.forEach(function(h, idx, arr) {
                        ...
                        var offset = typeof h[2] == 'string' ? 0 : -1;
                        if (offset < 0) {
                        }
                        if (!h[3+offset]) {
                            // both handlerClass and hooks are not specified
                            h[3+offset] = {pre: preHooks};
                        } else if (typeof h[3+offset] == 'function') {
                            if (!h[4+offset]) {
                                // sub-url has no hooks defined
                                h[4+offset] = {pre: preHooks};
                            } else {
                                // sub-url has its own hooks, merge
    ...
    get: function(pattern, handleFunction, handlerClass, hooks) {
        return this.add('GET', pattern, handleFunction, handlerClass, hooks);
    },

    post: function(pattern, handleFunction, handlerClass, hooks) {
        return this.add('POST', pattern, handleFunction, handlerClass, hooks);
    },

    put: function(pattern, handleFunction, handlerClass, hooks) {
        return this.add('PUT', pattern, handleFunction, handlerClass, hooks);
    },

    del: function(pattern, handleFunction, handlerClass, hooks) {
        return this.add('DELETE', pattern, handleFunction, handlerClass, hooks);
    },

    head: function(pattern, handleFunction, handlerClass, hooks) {
        return this.add('HEAD', pattern, handleFunction, handlerClass, hooks);
    },

    notFound: function(pattern, handleFunction, handlerClass, hooks) {
        return this.add('NOTFOUND', pattern, handleFunction, handlerClass, hooks);
    },
...
function _build(urls) {
    ...
    for(var type in ret) {
        if (!(type in ['PREFIX', 'NOTFOUND'])) {
            if (ret.PREFIX) ret[type] = ret[type].concat(ret.PREFIX);
        }
    }
    return ret;
}