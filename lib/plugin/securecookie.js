  ...
  attach: function (core, options) {
    ...
    core.add('writeHead', function (statusCode, headers, next) {
      if (headers.hasOwnProperty(cookieName)) {
        ...
        cookieOption.expires = originalCookie[1];
        // set other cookie option if any
        var option = originalCookie[3];
        if (option) {
          if (option.path) {
            cookieOption.path = option.path;
          }
          if (option.domain) {
            cookieOption.domain = option.domain;
          }
        }
        // make the cookie
        ...
        if (!setCookie) {
          ...
          throw new Error("Can not add cookie value to: " + JSON.stringify(setCookie));
        }
