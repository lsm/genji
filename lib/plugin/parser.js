...
module.exports = {
  ...
  attach: function (core, options) {
    ...
    return function (request, response, go) {
      ...
      if (urlObj.query) {
        injector.regDependency('query', parseQuerystring(urlObj.query));
      }
      ...
      if (request.method === 'POST' || request.method === 'PUT' || request.method === 'DELETE') {
        ...
        request.on("end", function () {
          injector.regDependency('data', buff);
          if (/application\/x-www-form-urlencoded/.test(contentType)) {
            injector.regDependency('params', parseQuerystring(buff));
          } else if (/json|javascript/.test(contentType)) {
            try {
              injector.regDependency('json', JSON.parse(buff));
            } catch (e) {}
          }
          go();
        });
